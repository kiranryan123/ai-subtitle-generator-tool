    ...
    try:
        ...
        for samples in audio_chunks(config.audio, stop_flag):
            ...
            if transcript.text:
                ...
                text = to_simplified(transcript.text) if source_language.startswith("zh") else transcript.text
                outbox.put(("caption", caption_id, text))
...
def main() -> None:
    ...
    def toggle_pause(_event=None) -> None:
        if paused.is_set():
            ...
            panel.set_status("正在监听 / Listening")
        else:
            paused.set()
